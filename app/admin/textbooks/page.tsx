...
export default function AdminTextbooksPage() {
  ...
  const fetchItems = async (uid: string) => { setLoading(true); const { data } = await supabase.from('text_books').select('*').eq('unit_id', uid).order('created_at', { ascending: false }); if (data) setItems(data); setLoading(false) }
  ...
  const handleDeleteUnit = async (uid: string) => { if (!confirm('Delete unit and textbooks?')) return; await supabase.from('text_books').delete().eq('unit_id', uid); await supabase.from('units').delete().eq('id', uid); if (selectedStandard) fetchUnits(selectedStandard.id) }
  ...
  const handleSaveItem = async () => {
    if (!itemForm.title || !itemForm.file_url || !selectedUnit || !selectedStandard) return
    setItemSaving(true)
    const p = { title: itemForm.title, description: itemForm.description || null, file_url: itemForm.file_url, chapter: itemForm.chapter || null, author: itemForm.author || null, unit_id: selectedUnit.id, standard_id: selectedStandard.id, uploaded_by: user?.id }
    if (editingId) await supabase.from('text_books').update(p).eq('id', editingId); else await supabase.from('text_books').insert(p)
    setItemForm({ title: '', description: '', file_url: '', chapter: '', author: '' }); setEditingId(null); setItemDialogOpen(false); setItemSaving(false); fetchItems(selectedUnit.id)
  }

  const handleEditItem = (t: TextBook) => { setItemForm({ title: t.title, description: t.description || '', file_url: t.file_url, chapter: t.chapter || '', author: t.author || '' }); setEditingId(t.id); setItemDialogOpen(true) }
  const handleDeleteItem = async (id: string) => { if (!confirm('Delete?')) return; await supabase.from('text_books').delete().eq('id', id); if (selectedUnit) fetchItems(selectedUnit.id) }
  ...
  return (
    <div className="min-h-screen">
      <AdminHeader title="Textbooks" subtitle="Manage textbook PDFs by standard and unit" />
      <div className="p-6 space-y-6">
        ...
        : level === 'standards' ? (
          ...
          <div className="space-y-4">
            ...
            {units.length === 0 ? <div className="text-center py-16"><FolderOpen className="h-16 w-16 mx-auto text-muted-foreground/40 mb-4" /><h3 className="text-lg font-semibold mb-2">No Units Yet</h3><p className="text-muted-foreground">Add units to organize textbooks</p></div>
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="text-lg font-bold flex items-center gap-2"><BookOpen className="h-5 w-5 text-blue-600" />Textbooks — {selectedUnit?.name}</h3>
              <Dialog open={itemDialogOpen} onOpenChange={(o) => { setItemDialogOpen(o); if (!o) { setEditingId(null); setItemForm({ title: '', description: '', file_url: '', chapter: '', author: '' }) } }}>
                <DialogTrigger asChild><Button><Plus className="mr-2 h-4 w-4" />Add Textbook</Button></DialogTrigger>
                <DialogContent>
                  <DialogHeader><DialogTitle>{editingId ? 'Edit' : 'Add'} Textbook</DialogTitle><DialogDescription>PDF link for {selectedUnit?.name}</DialogDescription></DialogHeader>
                  <div className="space-y-4 pt-4">
                    ...
                    <div className="grid gap-4 sm:grid-cols-2">
                      ...
                      <div className="space-y-2"><Label>Author</Label><Input value={itemForm.author} onChange={(e) => setItemForm({ ...itemForm, author: e.target.value })} /></div>
                    </div>
            ...
            {items.length === 0 ? <div className="text-center py-16"><BookOpen className="h-16 w-16 mx-auto text-muted-foreground/40 mb-4" /><h3 className="text-lg font-semibold mb-2">No Textbooks Yet</h3></div>
            : <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{items.map((t) => (
              <Card key={t.id} className="hover:shadow-md"><CardContent className="p-5">
                <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-blue-500/10 mb-3"><BookOpen className="h-5 w-5 text-blue-600" /></div>
                ...
                <div className="flex gap-2 mt-3">
                  <Button variant="outline" size="sm" className="flex-1" asChild><a href={t.file_url} target="_blank" rel="noopener noreferrer"><ExternalLink className="mr-1 h-3 w-3" />View PDF</a></Button>
                  <Button variant="outline" size="sm" onClick={() => handleEditItem(t)}><Edit className="h-3 w-3" /></Button>
                  <Button variant="ghost" size="sm" className="text-destructive" onClick={() => handleDeleteItem(t.id)}><Trash2 className="h-3 w-3" /></Button>
                </div>
              </CardContent></Card>
            ))}</div>}
          </div>
